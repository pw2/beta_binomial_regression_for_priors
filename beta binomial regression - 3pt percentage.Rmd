...
## Accounting for 3pt Shoot Attempts

* The outcome variable is binomial (successes and failures) so we will use a beta-binomial regression to estimate three point percent while controling for three point shot attempts
...
**NOTE:** sigma will be the same for all players, representing a variance that we expect the population to similarly to exhibit
...
* Notice that those with more 3 point attempts are close to the red line, representing perfect agreement between the two estimates(intercept = 0, slope = 1) while those with less atempts are pulled further down.
...
* Now we have those with a small number of attempts shrunk to a smaller prior three point percetage than those with a larger number of attempts.


## Make a prediction for a new player who has 10 three point attempts
